          ...
          return GetBuilder<CartController>(
            builder: (cartController) {
              return GridView.builder(
                ...
                // Filter products to show only those with a rating_count >= 4
                itemCount: productController.productData.where((product) {
                  var ratingCount = product["average_rating"];
                  return ratingCount != null && ratingCount =="5.00";
                }).length + (productController.isMoreDataAvailable ? 1 : 0),
                itemBuilder: (_, index) {
                  // Get the filtered products with rating_count >= 4
                  var filteredProducts = productController.productData.where((product) {
                    var ratingCount = product["average_rating"];
                    return ratingCount != null && ratingCount == "5.00";
                  }).toList();
